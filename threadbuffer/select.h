...
namespace Skate {
    ...
    public:
        ...
        void close_all() {
            ...
            for (SocketDescriptor fd = 0; fd < max_descriptor + 1; ++fd) {
                if (watching(fd))
                    close(fd);
            }
...
namespace Skate {
    ...
    public:
        ...
        int close(SocketDescriptor fd) {
            unwatch(fd);
            return closesocket(fd)? WSAGetLastError(): 0;
        }

        // Closes all file descriptors in the set, unwatching them from all watch types
        void close_all() {
            for_all_descriptors([](SocketDescriptor fd, WatchFlags) {
                closesocket(fd);
            }, master_read_set, master_write_set, master_except_set);
