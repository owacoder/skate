...
namespace Skate {
    ...
    namespace impl {
        ...
        // std::string helper specialization
        template<typename T>
        class InputOutputBufferReadHelper<T, std::basic_string<T>> {
            std::basic_string<T> &c;

        public:
            InputOutputBufferReadHelper(std::string &c) : c(c) {}

            void read_from_buffer_into(InputOutputBuffer<T> &buffer, size_t max) {
                c.reserve(c.size() + std::min(max, buffer.size()));
                buffer.read(max, [&](const T *data, size_t count) {
                    c.append(data, count);
                });
            }
            void read_all_from_buffer_into(InputOutputBuffer<T> &buffer) {
                c.reserve(c.size() + buffer.size());
                buffer.read_all([&](const T *data, size_t count) {
                    c.append(data, count);
                });
            }
        ...
        // std::vector<T> helper specialization
        template<typename T>
        class InputOutputBufferReadHelper<T, std::vector<T>> {
            std::vector<T> &c;
        ...
        public:
            ...
            void read_from_buffer_into(InputOutputBuffer<T> &buffer, size_t max) {
                c.reserve(c.size() + std::min(max, buffer.size()));
                buffer.read(max, [&](const T *data, size_t count) {
                    c.insert(c.end(), data, data + count);
                });
            }
            void read_all_from_buffer_into(InputOutputBuffer<T> &buffer) {
                c.reserve(c.size() + buffer.size());
